$(function() {
  ...
  if (!game.Name)
    return Log.die("Game was not set-up properly", "Missing Game Name");
  ...
  Game.save = function() {
    Memory.save(Game.Name, Game.state);
  };
  ...
  Game.Move = function(location) {
    ...
    console.log("Leaving", Game.state.location);
    Game.state.location = location;
    console.log("Entering", Game.state.location);
  ...
  Game.findLocation = function(shortName) {
    ...
    if (locs.length != 1)
      Log.die(
        "Game location data is corrupted",
        "The Game could not locate '" +
          shortName +
          ". searching for it returned " +
          locs.length +
          " locations"
      );
    else return locs[0];
  ...
  Game.consoleSpill = function() {
    while (Game.consoleQueue.length) {
      Game.console
        .children()
        .last()
        .append(Game.consoleQueue.splice(0, 1));
  ...
  Game.consoleSpill();
  Game.say(
    "You can enter commands such as 'Go to ...', 'Pick up ...', 'Look Around'"
  );
  Game.say("");
